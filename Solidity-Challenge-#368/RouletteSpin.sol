...
contract RouletteSpin {
    ...
    function spin() external payable {
        require(msg.value == 10 ether, 'You must send exactly 10 ether to play'); // Must send 10 ether to play
        require(block.timestamp != lastSpinTime, 'Only one transaction allowed per block'); // Only one spin per block
