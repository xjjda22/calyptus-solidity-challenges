...
describe("TicketingSystemFix", function () {
    ...
    it("should buy tickets correctly", async function () {
        ...
        await expect(
            ticketingSystem.connect(addr1).buyTicketsBatch(addr1.address, ids, quantities, { value: ethers.utils.parseEther("0.8") })
        ).to.not.be.reverted;
    ...
    it("should revert if max supply is reached", async function () {
        ...
        await expect(
            ticketingSystem.connect(addr1).buyTicketsBatch(addr1.address, ids, quantities, { value: ethers.utils.parseEther("0.3") })
    ...
    it("should revert if incorrect price is paid", async function () {
        ...
        await expect(
            ticketingSystem.connect(addr1).buyTicketsBatch(addr1.address, ids, quantities, { value: ethers.utils.parseEther("0.2") })
